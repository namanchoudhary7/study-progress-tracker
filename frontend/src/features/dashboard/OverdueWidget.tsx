  ...
  return (
    <div className="space-y-2 text-sm">
      {data.due_reviews_count > 0 && (
        <p>
          <Link to="/review" className="font-medium text-accent-600 hover:underline dark:text-accent-400">
            <span className="font-mono tabular-nums">{data.due_reviews_count}</span> topic{data.due_reviews_count === 1 ? "" : "s"} due for review
          </Link>
        </p>
      )}
